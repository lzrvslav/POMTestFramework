...
public class Header {
    ...
    // Locates the profile link using its ID and initializes it as a WebElement
    @FindBy(id = "nav-link-profile")
    private WebElement profileLink;

    // Locates the login link using its ID and initializes it as a WebElement
    @FindBy(id = "nav-link-login")
    private WebElement loginLink;

    // Locates the home link using its ID and initializes it as a WebElement
    ...
    private void waitAndClick(WebElement element) {
        try {
            // Create an explicit wait with a timeout of 3 seconds
            ...
            // Fail the test if the element does not become clickable within the timeout
            Assert.fail("Header navigation link is not found. Inner exception: " + exception);
        }

        // Click on the element once it is confirmed to be clickable
        element.click();
    }
    ...
    public void clickMenuLink(String menuItem) {
        ...
        switch (menuItem.toLowerCase()) {
            ...
            case "home":
                ...
                break;
            default:
                throw new IllegalArgumentException(menuItem + " menu option is not supported");
        }
        WebDriverWait pageLinkWait = new WebDriverWait(this.webDriver, Duration.ofSeconds(3));
